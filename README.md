...
MEDIACRAWLER_DIR=C:/Users/Admin/Desktop/MultiAgent for Trend/MediaCrawler-main/MediaCrawler-main
```
...
> **重要：** 根目录的 `requirements.txt` 是项目的唯一依赖清单，已合并 backend 和 MediaCrawler 的全部依赖。`backend/requirements.txt` 和 `MediaCrawler-main/.../requirements.txt` 仅供参考，无需单独安装。
...
```powershell
cd MediaCrawler-main\MediaCrawler-main
...
│
└── MediaCrawler-main/MediaCrawler-main/  # MediaCrawler 爬虫引擎
...
本项目仅供学习和研究目的使用。MediaCrawler 部分遵循其原始 [NON-COMMERCIAL LEARNING LICENSE](MediaCrawler-main/MediaCrawler-main/LICENSE)。
